...
func NewFileLogger(levelStr, fp, fn string, maxSize int64) *FileLogger {
	logLevel, err := ParesLogLevel(levelStr)
	if err != nil {
		panic(err)
	}
	fl := &FileLogger{
...
func (f *FileLogger) writeLogBackground() {
	...
	for {
		...
		case logTmp := <-f.logChan:
			// 把日志拼出来
			logStr, err := getLogString(logTmp.level)
			if err != nil {
				fmt.Printf("get log level failed err:%s ", err)
				return
			}
			logInfo := fmt.Sprintf("[%s] [%s] [%s:%s:%d] %s\n", logTmp.timestamp, logStr, logTmp.fileName, logTmp.funcName, logTmp.line, logTmp.msg)
...
// 记录日志的方法
func (f *FileLogger) logMsg(lv LogLevel, format string, a ...interface{}) {
	if f.enable(lv) {
...
func (f *FileLogger) Debug(format string, a ...interface{}) {
	f.logMsg(DEBUG, format, a...)
}

// Info 表明消息在粗粒度级别上突出强调应用程序的运行过程
func (f *FileLogger) Info(format string, a ...interface{}) {
	f.logMsg(INFO, format, a...)
}

// Warning 表明会出现潜在错误的情形
func (f *FileLogger) Warning(format string, a ...interface{}) {
	f.logMsg(WARAING, format, a...)
}

// Error 指出虽然发生错误事件，但仍然不影响系统的继续运行
func (f *FileLogger) Error(format string, a ...interface{}) {
	f.logMsg(ERROR, format, a...)
}

// Fatal 指出每个严重的错误事件将会导致应用程序的退出
func (f *FileLogger) Fatal(format string, a ...interface{}) {
	f.logMsg(FATAL, format, a...)
}
